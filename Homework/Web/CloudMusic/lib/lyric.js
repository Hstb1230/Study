        ...
        for (var i = 0; i < lines.length; i++) {
          ...
          if (result) {
            ...
            if (txt) {
              ...
              let minute = parseInt(result[1], 10);
              let second = parseInt(result[2], 10);
              let microSecond = result[3] || '0';
              // 修正毫秒数只有两位导致时间偏差的情况
              if(microSecond.length < 3) 
                microSecond *= 10;
              else 
                microSecond = parseInt(microSecond, 10);
              // console.log(`${minute}:${second}.${microSecond} -> ${minute * 60 * 1000 + second * 1000 + microSecond}`)
              this.lines.push({
                time: minute * 60 * 1000 + second * 1000 + microSecond,
                txt: txt
              });
            }
          }
